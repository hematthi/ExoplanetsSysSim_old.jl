...
function calc_distance(ss1::ExoplanetsSysSim.CatalogSummaryStatistics, ss2::ExoplanetsSysSim.CatalogSummaryStatistics, return_KS_or_AD::String ; AD_mod::Bool=false, all_dist::Bool=false, save_dist::Bool=true)
    ...
    d_KS[7] = ksstats(ss1.stat["duration_ratio_non_mmr_list"], ss2.stat["duration_ratio_non_mmr_list"])[5]
    d_KS[8] = ksstats(ss1.stat["duration_ratio_near_mmr_list"], ss2.stat["duration_ratio_near_mmr_list"])[5]
    d_KS[9] = ksstats(ss1.stat["depth list"], ss2.stat["depth list"])[5]
    d_KS[10] = ksstats(ss1.stat["depth above list"], ss2.stat["depth above list"])[5]
    d_KS[11] = ksstats(ss1.stat["depth below list"], ss2.stat["depth below list"])[5]
    d_KS[12] = ksstats(ss1.stat["radius_ratio_list"], ss2.stat["radius_ratio_list"])[5]
    d_KS[13] = ksstats(ss1.stat["radius_ratio_above_list"], ss2.stat["radius_ratio_above_list"])[5]
    d_KS[14] = ksstats(ss1.stat["radius_ratio_below_list"], ss2.stat["radius_ratio_below_list"])[5]
    d_KS[15] = ksstats(ss1.stat["radius_ratio_across_list"], ss2.stat["radius_ratio_across_list"])[5]
    ...
    d_AD[7] = ADdist(ss1.stat["duration_ratio_non_mmr_list"], ss2.stat["duration_ratio_non_mmr_list"])
    d_AD[8] = ADdist(ss1.stat["duration_ratio_near_mmr_list"], ss2.stat["duration_ratio_near_mmr_list"])
    d_AD[9] = ADdist(ss1.stat["depth list"], ss2.stat["depth list"])
    d_AD[10] = ADdist(ss1.stat["depth above list"], ss2.stat["depth above list"])
    d_AD[11] = ADdist(ss1.stat["depth below list"], ss2.stat["depth below list"])
    d_AD[12] = ADdist(ss1.stat["radius_ratio_list"], ss2.stat["radius_ratio_list"])
    d_AD[13] = ADdist(ss1.stat["radius_ratio_above_list"], ss2.stat["radius_ratio_above_list"])
    d_AD[14] = ADdist(ss1.stat["radius_ratio_below_list"], ss2.stat["radius_ratio_below_list"])
    d_AD[15] = ADdist(ss1.stat["radius_ratio_across_list"], ss2.stat["radius_ratio_across_list"])
...
function calc_distance_Kepler(ss1::ExoplanetsSysSim.CatalogSummaryStatistics, return_KS_or_AD::String ; AD_mod::Bool=false, all_dist::Bool=false, save_dist::Bool=true)
    ...
    d_KS[7] = ksstats(ss1.stat["duration_ratio_non_mmr_list"], xi_non_mmr_confirmed)[5]
    d_KS[8] = ksstats(ss1.stat["duration_ratio_near_mmr_list"], xi_near_mmr_confirmed)[5]
    d_KS[9] = ksstats(ss1.stat["depth list"], D_confirmed)[5]
    d_KS[10] = ksstats(ss1.stat["depth above list"], D_above_confirmed)[5]
    d_KS[11] = ksstats(ss1.stat["depth below list"], D_below_confirmed)[5]
    d_KS[12] = ksstats(ss1.stat["radius_ratio_list"].^2, D_ratio_confirmed)[5] #simulations save radius ratios while we computed transit duration ratios from the Kepler catalog
    d_KS[13] = ksstats(ss1.stat["radius_ratio_above_list"].^2, D_ratio_above_confirmed)[5]
    d_KS[14] = ksstats(ss1.stat["radius_ratio_below_list"].^2, D_ratio_below_confirmed)[5]
    d_KS[15] = ksstats(ss1.stat["radius_ratio_across_list"].^2, D_ratio_across_confirmed)[5]
    ...
    d_AD[7] = ADdist(ss1.stat["duration_ratio_non_mmr_list"], xi_non_mmr_confirmed)
    d_AD[8] = ADdist(ss1.stat["duration_ratio_near_mmr_list"], xi_near_mmr_confirmed)
    d_AD[9] = ADdist(ss1.stat["depth list"], D_confirmed)
    d_AD[10] = ADdist(ss1.stat["depth above list"], D_above_confirmed)
    d_AD[11] = ADdist(ss1.stat["depth below list"], D_below_confirmed)
    d_AD[12] = ADdist(ss1.stat["radius_ratio_list"].^2, D_ratio_confirmed) #simulations save radius ratios while we computed transit duration ratios from the Kepler catalog
    d_AD[13] = ADdist(ss1.stat["radius_ratio_above_list"].^2, D_ratio_above_confirmed)
    d_AD[14] = ADdist(ss1.stat["radius_ratio_below_list"].^2, D_ratio_below_confirmed)
    d_AD[15] = ADdist(ss1.stat["radius_ratio_across_list"].^2, D_ratio_across_confirmed)
...
function target_function(active_param::Vector{Float64}, use_KS_or_AD::String, Kep_or_Sim::String ; AD_mod::Bool=false, weights::Vector{Float64}=ones(15), all_dist::Bool=false, save_dist::Bool=true)
...
function target_function_transformed_params(active_param_transformed::Vector{Float64}, transformed_indices::Vector{Int64}, A::Vector{Float64}, B::Vector{Float64}, C::Vector{Float64}, use_KS_or_AD::String, Kep_or_Sim::String ; AD_mod::Bool=false, weights::Vector{Float64}=ones(15), all_dist::Bool=false, save_dist::Bool=true)
